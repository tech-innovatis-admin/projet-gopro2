...
import { listContractAuditLogs } from "@/src/lib/api/endpoints/auth";
...
const ENTITY_FILTER_OPTIONS: EntityFilterOption[] = [
  ...
  },
  {
    ...
    label: "Receitas",
    entityType: "incomes",
    description: "Eventos de receitas e entradas financeiras do contrato.",
  },
  {
    key: "EXPENSES",
    label: "Despesas",
    entityType: "expenses",
    description: "Eventos de despesas e saidas financeiras do contrato.",
  },
];
...
export default function ContractAuditPage() {
  ...
  const loadLogs = useCallback(async () => {
    ...
    try {
      const response = await listContractAuditLogs({
        scope: "CONTRACTS",
        contractId,
  ...
  }, [canView, contractId, entityTypeFilter, currentPage]);
    ...
    () =>
      Object.fromEntries(
        Object.values(budgetItemsById).map((item) => [
          item.id,
          buildBudgetItemReferencePresentation(item, budgetCategoryLabelsById[item.categoryId] || null),
        ])
      ),
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-zinc-500">
                {ENTITY_FILTER_OPTIONS.find((option) => option.entityType === entityTypeFilter)?.description ||
                  "Mostra os eventos do contrato conforme o atalho selecionado."}
              </p>
            </div>
          </section>
              ...
              <div className="space-y-5">
                <div className="space-y-4">
                  {logs.map((log, index) => {
                    ...
                    const enhancedChanges = enhanceBudgetReferenceChanges(parseChanges(log.alteracoesJson), {
                    ...
                    return (
                      <div key={log.auditId || log.id} className="relative pl-8">
                        ...
                        {hasConnector && (
                          <span className="absolute left-[7px] top-8 h-[calc(100%-12px)] w-px bg-zinc-200" />
                        )}
                        ...
                        >
                          <dl className="mt-4 grid gap-3 text-sm md:grid-cols-2">
                            ...
                            <div>
                              ...
                              <dd className="text-xs text-zinc-600">{contractLabel}</dd>
                            </div>
                          </dl>
                        </AuditLogCard>
                      </div>
