# Editar Parceiro Page — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Reescrever `editar/page.tsx` do módulo de parceiros para ter paridade completa com `NovoParceiroModal.tsx` — formatadores, seletor de tipo, ViaCep, IBGE, `useFormApiErrors`, `useModalCloseGuard`.

**Architecture:** Arquivo único (`page.tsx`). Toda a lógica é extraída do `NovoParceiroModal.tsx` e adaptada para contexto de página: sem backdrop/chrome de modal, dados carregados do backend no mount, submit via `updatePartner`. O `useModalCloseGuard` é usado com `isOpen: true` fixo e `onClose` apontando para `router.push`.

**Tech Stack:** Next.js App Router, React hooks, `useFormApiErrors`, `useModalCloseGuard`, `Dropdown` (Radix UI), `ConfirmDiscardModal`, ViaCep, IBGE (`fetchCitiesByState`), `formatCnpj` / `formatPhone` / `formatZipCode`.

---

## Arquivo modificado

- `src/app/(dashboard)/parceiros/[parceiroId]/editar/page.tsx` — reescrita completa

## Decisões de design

| Ponto | Decisão |
|-------|---------|
| `useModalCloseGuard` | `isOpen: true` fixo (página nunca "fecha"), `onClose = router.push(/parceiros/{id})` |
| CEP ao carregar | Inicializa como `""` — `mapPartnerToParceiro` não mapeia CEP do backend; usuário pode buscar novo CEP para atualizar endereço |
| CNPJ ao carregar | `mapPartnerToParceiro` já retorna CNPJ formatado; o form exibe e `mapParceiroFormToPartnerRequestDTO` remove máscara no submit |
| Telefone ao carregar | Passar por `formatPhone` ao inicializar o form para exibir máscara correta |
| Detecção de alterações | `JSON.stringify(form) !== JSON.stringify(originalForm)` — igual à abordagem atual |
| `observacoes` | Campo não está no `PartnerResponseDTO`/`PartnerRequestDTO`; incluir no form como string vazia; não enviar no payload (mapper já ignora) |

---

## Task 1: Reescrever `editar/page.tsx`

**Files:**
- Modify: `src/app/(dashboard)/parceiros/[parceiroId]/editar/page.tsx` (reescrita completa)
...
```powershell
cd projet-gopro2; npx tsc --noEmit 2>&1 | Select-Object -First 40
```

Esperado: sem erros relacionados ao arquivo editado.

- [ ] **Step 3: Commit**

```bash
git add src/app/\(dashboard\)/parceiros/\[parceiroId\]/editar/page.tsx
git commit -m "fix: reescrever página de edição de parceiro com formatadores, IBGE e ViaCep"
```

---

## Checklist de paridade com NovoParceiroModal

- [ ] Seletor visual IFES/Fundação com ícones e highlight
- [ ] `formatCnpj` no onChange do campo CNPJ
- [ ] `formatPhone` no onChange do campo telefone
- [ ] `formatZipCode` + lookup ViaCep ao digitar CEP
- [ ] `Dropdown` do Radix UI para UF (lista `UF_LIST`)
- [ ] `fetchCitiesByStateLookup` para municípios com `Dropdown` pesquisável
- [ ] Fallback `allowManualMunicipioEntry` para input de texto quando IBGE falha
- [ ] `useFormApiErrors` com mesmo `fieldMap` do modal
- [ ] `useModalCloseGuard` + `ConfirmDiscardModal` para cancelar com alterações pendentes
- [ ] Erro global da API renderizado acima dos botões
- [ ] Loading e error states para o carregamento inicial
